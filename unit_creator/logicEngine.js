const logicEngine = {
  ...
  calculateUnitPoints: function (stats, activeKws, keywordsDb) {
    try {
      ...
      // --- 4. FINAL PILLAR MERGE ---
      const rawBv = baseWndVal * omSave * omMove * omArmor * typeBvMod * bvMult + bvFlat;
      const rawAv = baseAvVal * omCv * typeAvMod * titanPenalty * avMult + avFlat;

      const bvSatMult = Math.pow(Math.max(rawBv, 20) / 20, 0.15);
      const avSatMult = Math.pow(Math.max(rawAv, 20) / 20, 0.25);
      ...
      log += `   Anchor Ratio: ${Math.max(rawBv, 20).toFixed(2)} / 20.00\n`;
      log += `   BV Saturation: (Ratio)^0.15 = ${bvSatMult.toFixed(2)}x\n`;
      ...
      log += `     - Base Action Value: (${rsDiceScore.toFixed(2)} + ${msDiceScore.toFixed(2)}) * 1.75 = ${baseAvVal.toFixed(2)}\n`;
      ...
      log += `   Anchor Ratio: ${Math.max(rawAv, 20).toFixed(2)} / 20.00\n`;
      log += `   AV Saturation: (Ratio)^0.25 = ${avSatMult.toFixed(2)}x\n`;
